...
class ForgetPasswordFormTest < PasswordResets
...
class PasswordResetForm < PasswordResets
  def setup
    ...
    @reset_user = assigns(:user) # get user from magic thin air  jk from PasswordResetsController#create
  end
end
...
class PasswordFormTest < PasswordResetForm
  ...
  test 'reset with wrong email' do
    get edit_password_reset_path(@reset_user.reset_token, email: '') # empty email
    assert_redirected_to root_url
  end
  ...
  test 'reset with right email but wrong token' do
    get edit_password_reset_path(User.new_token, email: @reset_user.email)
    assert_redirected_to root_url
  end
...
class PasswordUpdateTest < PasswordResetForm
  test 'update with invalid password and confirmation' do
    patch password_reset_path(@reset_user.reset_token),
          params: { email: @reset_user.email, user: { password: 'foobar', password_confirmation: 'barbaz' } }
  ...
  test 'update with empty password' do
    patch password_reset_path(@reset_user.reset_token),
          params: { email: @reset_user.email, user: { password: '', password_confirmation: '' } }
  ...
  test 'update with valid password and confirmation' do
    patch password_reset_path(@reset_user.reset_token),
          params: { email: @reset_user.email, user: { password: '123123', password_confirmation: '123123' } }
    assert is_logged_in?
    assert_not flash.empty?
